...
import com.zhixiangli.smartgomoku.SmartGomoku;
...
public class GomokuFXManager {
    ...
    public GomokuFXManager() {
        for (int row = 0; row < Chessboard.DEFAULT_SIZE; ++row) {
            for (int column = 0; column < Chessboard.DEFAULT_SIZE; ++column) {
                this.chessboardProperty[row][column] = new SimpleIntegerProperty(
    ...
    public void makeMove(int row, int column) {
        ...
        if (GomokuReferee.isWin(chessboard, row, column)) { // if win.
            this.chessboardStateProperty
                .set(ChessType.BLACK == this.currentChessType ? ChessboardState.BLACK_WIN
                    : ChessboardState.WHITE_WIN);
            ...
            this.currentPlayerProperty
                .set(ChessType.BLACK == currentChessType ? this.blackPlayerStrategyClass
