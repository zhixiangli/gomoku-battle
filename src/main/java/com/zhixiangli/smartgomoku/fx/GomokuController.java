...
import com.zhixiangli.smartgomoku.ai.GomokuAlphaBetaPruning;
...
public class GomokuController {
    ...
    public void initialize() throws IOException {
        ...
        SimpleObjectProperty<ChessboardState> chessboardStateProperty = this.gomokuManager
            .getChessboardStateProperty();
        this.setAnnouncement(chessboardStateProperty.get());
        chessboardStateProperty.addListener(event -> this.setAnnouncement(chessboardStateProperty
            .get()));
        ...
        for (int row = 0; row < Chessboard.DEFAULT_SIZE; ++row) {
            for (int column = 0; column < Chessboard.DEFAULT_SIZE; ++column) {
                this.chessboardGridPane.getChildren().add(
                    new GomokuCellPane(row, column, this.gomokuManager));
            }
        }
    ...
    private void setPlayerStrategy() {
        Class<? extends GomokuAI> blackPlayerStrategyClass = UIConstant.COMPUTER_PLAYER
            .equals(this.blackPlayerChoiceBox.getSelectionModel().getSelectedItem()) ? GomokuAlphaBetaPruning.class
            : null;
        Class<? extends GomokuAI> whitePlayerStrategyClass = UIConstant.COMPUTER_PLAYER
            .equals(this.whitePlayerChoiceBox.getSelectionModel().getSelectedItem()) ? GomokuAlphaBetaPruning.class
            : null;
