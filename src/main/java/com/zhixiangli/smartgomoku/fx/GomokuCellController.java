...
class GomokuCellController {
    ...
    public void initialize() {
        ...
        this.cellPane.disableProperty().bind(
            this.gomokuManger
                .getChessboardStateProperty()
                .isNotEqualTo(ChessboardState.GAME_ON)
                .or(this.gomokuManger.getChessboardProperty(row, column).isNotEqualTo(
                    ChessType.EMPTY.ordinal())));
        ...
        this.cellCircle.radiusProperty().bind(
            Bindings.min(cellPane.widthProperty(), cellPane.heightProperty()).divide(2.5));
        
        // the chess type of this cell, and add listener when the chess type
        // is changed.
        SimpleIntegerProperty chessTypeProperty = this.gomokuManger.getChessboardProperty(row,
            column);
    ...
    private void updateCell(SimpleIntegerProperty chessTypeProperty) {
        if (ChessType.EMPTY.ordinal() == chessTypeProperty.get()) {
            ...
            this.cellCircle
                .setFill(ChessType.BLACK.ordinal() == chessTypeProperty.get() ? Color.BLACK
                    : Color.WHITE);
        }
    }
